...
public class AzureMediaServiceService{
    ...
    public StreamingLocatorResponse uploadAndGetStreamingLocatorFromURL(String url) {
        ...
        String outputAssetName = "output-" + uniqueness;
        String inputAssetName = "input-" + uniqueness;
        String fileName="stream-"+uniqueness;

        try {

            // Create a new input Asset and upload the specified local video file into it.
            Asset inputAsset = createInputAssetFromURL(manager, resourceGroup, accountName, inputAssetName,
                    fileName,url);
    ...
    private Asset createInputAssetFromURL(MediaManager manager, String resourceGroupName, String accountName,
                                   String assetName, String fileName, String url) throws Exception {
        Asset asset;
        try {
            // In this example, we are assuming that the asset name is unique.
            // If you already have an asset with the desired name, use the Assets.getAsync method
        ...
        blob.upload(targetStream, targetStream.readAllBytes().length);
